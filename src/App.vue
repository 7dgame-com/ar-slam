    ...
    <Transition name="handshake-fade">
      ...
      <div v-if="showHandshake && !inIframe" key="warning" class="handshake-overlay">
        <div class="handshake-card">
          ...
          <a v-if="showDiagnosticsLink" href="/api-diagnostics" class="diag-link">
            {{ $t('handshake.goToDiagnostics') }}
          </a>
        </div>
      </div>
      <!-- iframe 内：简单 loading -->
...
// 公开路由不需要 token 认证
const PUBLIC_ROUTES = import.meta.env.DEV ? ['/api-diagnostics'] : []
const isPublicRoute = computed(() => PUBLIC_ROUTES.some((p) => route.path.startsWith(p)))
const showDiagnosticsLink = import.meta.env.DEV
